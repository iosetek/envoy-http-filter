...
AuthorizationFilter::AuthorizationFilter(
  ...
  Stats::Counter& cache_misses)
    ...
    cache_misses_(cache_misses) {
      callbacks_ = std::make_unique<AuthorizationCallbacks>(
        [this](){acceptAuthorization(false);},
...
FilterDataStatus AuthorizationFilter::decodeData(Buffer::Instance& data, bool eos) {
  ...
  if (decoder_callbacks_->decodingBuffer() != nullptr) {
    j = nlohmann::json::parse(decoder_callbacks_->decodingBuffer()->toString() + data.toString(), nullptr, false);
  } else {
...
FilterTrailersStatus AuthorizationFilter::decodeTrailers(RequestTrailerMap&) {
  ...
  nlohmann::json j = nlohmann::json::parse(
    decoder_callbacks_->decodingBuffer()->toString(), nullptr, false);
...
void AuthorizationFilter::callAuthorizationService(const std::string& user) {
  ...
  async_client.send(
      std::move(request), *callbacks_, Http::AsyncClient::RequestOptions());
}
...
void AuthorizationFilter::denyAuthorization(bool cached) {
  ...
  Http::Code::Unauthorized, "Unauthorized", {}, absl::nullopt, "failed-to-authorize");
}
}
